...
##########
# Config:
universal_exit_key = "KEY_ESC" # Just in case the one in the config doesn't work. Set to something that isn't a keycode to disable, such as None or False.
...
def set_key(code, state=0, dev=None):
    key = ecodes.ecodes[code]
    dev.write(ecodes.EV_KEY, key, state)
    dev.syn()

def press_key(code, hold_time=hold_time_def, dev=None):
    set_key(code, 1, dev)
    sleep(hold_time)
    set_key(code, 0, dev)

def press_keys(keys, dev):
    for key in keys:
    ...
    if device in devices.keys():
        ...
        try:
            ...
            try:
                ...
                #    print(gen_log_msg(device_short, current_layer.get_string(), str(e)))
                                    ...
                                    if exit_cmd_tag in device_config:
                                        ...
                                        if isinstance(exit_cmd, str):
                                            try:
                                                ...
                                                print(gen_log_msg(device_short, current_layer.get_string(), f"Device [{device_short}] hit an error [{str(error)}] while running its exit command [{exit_cmd}], ignoring..."))
                                    break
                                else:
...
Requires python 3!
        ...
        if len(args) > 1:
            if "help" in args or "h" in args:
                ...
                if len(args) >= 3:
                    add_device(args[2], config_file)
                else:
                    print_usage_msg(True)
            elif "add-layer" in args:
                if len(args) >= 4:
                    add_layer(args[2], args[3], config_file)
                else:
                    print_usage_msg(True)
            elif "add-keybind" in args:
                if len(args) >= 7:
                    add_keybind(args[2], args[3], args[4], args[5], args[6], config_file)
                else:
                    print_usage_msg(True)
            else:
                print_usage_msg(True)
