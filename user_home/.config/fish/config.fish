...
alias install-searx='docker stop searx-1; docker rm -v searx-1; PORT=8888 docker run --name=searx-1 --restart=unless-stopped -d -v ~/ProgramFiles/searx:/etc/searx -p $PORT:8080 -e BASE_URL=http://localhost:$PORT/ searx/searx:latest' # --dns 127.0.0.1
alias install-searxng='docker stop searxng-1; docker rm -v searxng-1; PORT=8888 docker run --name=searxng-1 --restart=unless-stopped -d -v ~/ProgramFiles/searxng:/etc/searxng -p $PORT:8080 -e BASE_URL=http://localhost:$PORT/ searxng/searxng:latest' # --dns 127.0.0.1
...
alias uu-doom="doom --force sync; doom --force upgrade; doom --force sync; doom --force purge; doom --force sync; doom --force build; doom --force sync"
